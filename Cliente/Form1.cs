...
namespace Cliente
{
    ...
    public partial class FrmValidador : Form
    {
        ...
        private void btnIniciar_Click(object sender, EventArgs e)
        {
            ...
            // Crear el pedido con el nombre de usuario y la contraseña proporcionados por el usuario
            Protocolos protocolo = new Protocolos(new Pedido
            {
                Comando = "INGRESO",
                Parametros = new[] { usuario, contraseña }
            });

            // Llamar a HazOperacion para enviar la solicitud al servidor
            Respuesta respuesta = HazOperacion(protocolo);
            if (respuesta == null)
            {
                MessageBox.Show("Hubo un error", "ERROR");
                return;
        ...
        private Respuesta HazOperacion(Protocolos protocolo)
        {
            if (flujo == null)
            {
                MessageBox.Show("No hay conexión", "ERROR");
            ...
            {
                byte[] bufferTx = Encoding.UTF8.GetBytes(
                    protocolo.Pedido.Comando + " " + string.Join(" ", protocolo.Pedido.Parametros));
                ...
                // Pasando la respuesta procesada
                return new Respuesta
                {
                    Estado = partes[0],
                    Mensaje = string.Join(" ", partes.Skip(1).ToArray())
